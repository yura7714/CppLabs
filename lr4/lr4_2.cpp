#include <iostream>
#include <cstring>  // для strcpy и setlocale
#include <clocale>  // для LC_ALL
...
int main(void)
{
    ...
    // Выделение динамической памяти с помощью malloc (как на скриншоте)
    ...
    // Освобождение динамической памяти с помощью free (как на скриншоте)
    free(concatRes);
    free(str1);
    free(str2);
...
char* konkat(char* str1, char* str2) {
    ...
    char* concatStr = (char*)malloc(totalLen);
